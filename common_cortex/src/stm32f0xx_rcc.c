...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks)
{
  ...
  if((RCC->CFGR3 & RCC_CFGR3_CECSW) != RCC_CFGR3_CECSW)
  {
    ...
    RCC_Clocks->CECCLK_Frequency = HSI_VALUE / 244U;
  }
  else
  {
  ...
  if((RCC->CFGR3 & RCC_CFGR3_USART1SW) == 0x0U)
  {
  ...
  if((RCC->CFGR3 & RCC_CFGR3_USART2SW) == 0x0U)
  {
    /* USART Clock is PCLK */
  ...
  if((RCC->CFGR3 & RCC_CFGR3_USART3SW) == 0x0U)
  {
    /* USART Clock is PCLK */
  ...
  {
     if((RCC->CFGR3 & RCC_CFGR3_USART3SW) == RCC_CFGR3_USART3SW)
     {
    /* USART Clock is HSI Osc. */
...
FlagStatus RCC_GetFlagStatus(uint8_t RCC_FLAG)
{
  ...
  tmp = (uint32_t)RCC_FLAG >> 5U;
#if defined (__ICCARM__) 
#pragma cstat_suppress="MISRAC2012-Rule-14.3_a"
  if (tmp == 0U)               /* The flag to check is in CR register */
#else
  if (tmp == 0U)               /* The flag to check is in CR register */
#endif
  {
  ...
  else if (tmp == 1U)          /* The flag to check is in BDCR register */
  {
    statusreg = RCC->BDCR;
  }
  else if (tmp == 2U)          /* The flag to check is in CSR register */
  {
    statusreg = RCC->CSR;
  }
  ...
  tmp = ((uint32_t)RCC_FLAG & (uint32_t)FLAG_MASK);
