...
#define IT_MASK                   ((uint32_t)0x000000FFU)
...
void USART_Init(USART_TypeDef* USARTx, USART_InitTypeDef* USART_InitStruct)
{
  ...
  if ((USARTx->CR1 & USART_CR1_OVER8) != 0U)
  {
    /* (divider * 10) computing in case Oversampling mode is 8 Samples */
    divider = (uint32_t)((2U * apbclock) / (USART_InitStruct->USART_BaudRate));
    tmpreg  = (uint32_t)((2U * apbclock) % (USART_InitStruct->USART_BaudRate));
  }
  ...
  if (tmpreg >=  (USART_InitStruct->USART_BaudRate) / 2U)
  {
    divider++;
  } 
...
void USART_ITConfig(USART_TypeDef* USARTx, uint32_t USART_IT, FunctionalState NewState)
{
  ...
  usartreg = ((USART_IT) >> 0x08);
  
  /* Get the interrupt position */
  itpos = (USART_IT & IT_MASK) & 0x1FU;
  itmask = (0x01UL << itpos);
  
  if (usartreg == 0x02U) /* The IT is in CR2 register */
  {
    usartxbase += 0x04U;
  }
  else 
  {
     if(usartreg == 0x03U) /* The IT is in CR3 register */
     {
        usartxbase += 0x08U;
     }
  }
...
ITStatus USART_GetITStatus(USART_TypeDef* USARTx, uint32_t USART_IT)
{
  ...
  /* Get the USART register index */
  usartreg = USART_IT >> 0x08U;
  /* Get the interrupt position */
  itmask = (USART_IT & IT_MASK) & 0x1FU;
  itmask = 0x01UL << itmask;
